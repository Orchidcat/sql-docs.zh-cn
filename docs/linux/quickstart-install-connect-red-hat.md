...
在此快速入门教程中，你首先安装 SQL Server 2017 上 Red Hat Enterprise Linux (RHEL) 7.3 +。 然后通过**sqlcmd**连接并创建第一个数据库和运行查询。
...
你必须有一台内存至少2GB 并安装 RHEL 7.3 或 7.4 版本操作系统的机器。
...
若要在 RHEL 上配置 SQL Server，在终端运行以下命令来安装**mssql server**包：

> [!IMPORTANT]
> 如果你已经安装 CTP 或 RC 版本的 SQL Server 2017，在注册一个 GA 存储库之前必须先删除旧的存储库。 有关详细信息，请参阅[从预览存储库更改到 GA 存储库](sql-server-linux-change-repo.md)。
...
1. 软件包安装完成后，运行**mssql conf 安装**命令并按照操作提示设置 SA 密码，并选择你的版本。
   ...
   ```
   > [!TIP]
   > 如果你在本教程中尝试使用 SQL Server 2017，你可以自由的使用以下版本： 评估、 开发人员和快速。
